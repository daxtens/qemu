...
static int xive_tctx_kvm_get_state(XiveTCTX *tctx)
{
    ...
    *((uint64_t *) &tctx->regs[TM_QW1_OS + TM_WORD2]) = state[1];
    ...
    /* OPAL internal state quads. They hold word4 of the NVT structure
     * and a version. Only the backup of the IPB is of interest in
     * there but we should consider it as opaque.
     */
    tctx->opal[0] = state[2];
    tctx->opal[1] = state[3];
    return 0;
