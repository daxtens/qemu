                                          ...
                                          target_ulong *args)
{
    ...
    if (xive_router_get_ive(XIVE_ROUTER(spapr->xive), lisn, &ive)) {
        return H_P2;
    }
...
static target_ulong h_int_set_source_config(PowerPCCPU *cpu,
                                            ...
                                            target_ulong *args)
{
    XiveRouter *xrtr      = XIVE_ROUTER(spapr->xive);
    XiveIVE ive, new_ive;
    target_ulong flags    = args[0];
    target_ulong lisn     = args[1];
    ...
    if (priority == 0xff) {
        new_ive.w = IVE_VALID | IVE_MASKED;
        goto out;
    }

    if (flags & SPAPR_XIVE_SRC_MASK) {
        new_ive.w = ive.w | IVE_MASKED;
    } else {
        new_ive.w = ive.w & ~IVE_MASKED;
    }

    if (!spapr_xive_eq_is_valid(priority)) {
     ...
     * target.
     */
    if (spapr_xive_target_to_eq(xrtr, target, priority, &eq_blk, &eq_idx)) {
        return H_P3;
    }
    ...
    if (flags & SPAPR_XIVE_SRC_SET_EISN) {
        new_ive.w = SETFIELD(IVE_EQ_DATA, new_ive.w, eisn);
    }

out:
    if (xive_router_set_ive(XIVE_ROUTER(spapr->xive), lisn, &new_ive)) {
        return H_HARDWARE;
    }
                                            ...
                                            target_ulong *args)
{
    ...
    XiveIVE ive;
    XiveEQ eq;
    uint8_t eq_blk, vp_blk;
    uint32_t eq_idx, vp_idx;
    ...
    if (xive_router_get_eq(XIVE_ROUTER(spapr->xive), eq_blk, eq_idx, &eq)) {
    ...
    args[0] = spapr_xive_vp_to_target(spapr->xive, vp_blk, vp_idx);
                                         ...
                                         target_ulong *args)
{
    ...
    args[0] = xive->eq_base + (1ull << (eq_xsrc->esb_shift + 1)) * eq_idx;
                                           ...
                                           target_ulong *args)
{
    ...
    if (spapr_xive_target_to_vp(xrtr, target, &vp_blk, &vp_idx)) {
        return H_HARDWARE;
    }
...
out:
    /* Update EQ */
    if (xive_router_set_eq(xrtr, eq_blk, eq_idx, &eq)) {
        return H_HARDWARE;
    }
...
void spapr_dt_xive(sPAPRXive *xive, int nr_servers, void *fdt, uint32_t phandle)
{
    ...
    /* The following array is in sync with the 'spapr_xive_eq_is_valid'
